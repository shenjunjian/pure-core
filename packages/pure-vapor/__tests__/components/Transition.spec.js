...
function createAppearTestState(show, extraState = {}) {
...
describe('Transition', () => {
  ...
  test('treats null group owner key as absent', () => {
    const Child = defineVaporComponent({
      setup() {
        return [template(`<div>a</div>`)(), template(`<div>b</div>`)()]
      },
    })
  ...
  test('direct child with initial shown v-show should call appear after insertion', async () => {
    ...
    const data = ref({
      show: true,
      onBeforeAppear: el => calls.push(el.isConnected),
      onAppear: el => calls.push(el.isConnected),
    })
    const App = compile(
      `<template>
        <Transition
  ...
  test('dynamic slot branch swaps preserve persisted hooks for slot-root v-show', async () => {
    ...
    const { host, instance } = define(App).render()
    const transitionFragment = instance.block.block
